...
def analyze_uvvis_spectrum(x: np.ndarray, y: np.ndarray, energy: bool = False) -> dict:
    """Return the global absorbance maximum and its x position."""
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    mask = np.isfinite(x) & np.isfinite(y)
    x = x[mask]
    y = y[mask]
    if x.size == 0:
        raise ValueError("No valid UV-Vis points for peak analysis.")
    ...
    if preset in ("UV-VIS", "UV-VIS (eV)") and simple_datasets:
        ...
        with st.expander("UV–Vis peak summary", expanded=True):
            ...
            for name, (x, y) in simple_datasets.items():
                try:
                    row = {"Dataset": name}
                    row.update(analyze_uvvis_spectrum(x, y, energy=(preset == "UV-VIS (eV)")))
                    rows.append(row)
                except Exception as exc:
            ...
            st.caption("Peak is the highest absorbance value in the uploaded spectrum.")
        available_names = list(simple_datasets.keys())
