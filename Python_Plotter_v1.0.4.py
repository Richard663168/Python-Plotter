...
st.caption("Scientific data plotting and processing · Developed by Richard Li · v1.0.4")
    ...
    *,
    ...
    for ax, name in targets:
        ...
        if preset == "TRPL":
            ...
            if trpl_lifetime_display == "τ1 + τ2" and params.get("model") == "bi":
                tau1 = float(params.get("tau1", float("nan")))
                tau2 = float(params.get("tau2", float("nan")))
                lifetime_label = f"τ₁ = {tau1:.3g} ns, τ₂ = {tau2:.3g} ns"
            elif trpl_lifetime_display == "τ1 + τ2":
                tau = float(params.get("tau", tau_fit))
                lifetime_label = f"τ = {tau:.3g} ns"
            else:
                lifetime_label = f"τ̄ = {tau_fit:.3g} ns"
            ax.plot(
                d["t"], d["I"],
                linestyle="None", marker="o", markersize=4,
                color=color, label=f"{name}  ({lifetime_label})",
            )
            ax.plot(
                d["fit_t"], d["fit_I"],
...
with general_tab:
    ...
    if preset == "CD" and file_list:
        ...
        st.subheader("TRPL Options")
        trpl_lifetime_display = st.radio(
            "Displayed lifetime",
            ["τavg", "τ1 + τ2"],
            horizontal=True,
            help=(
                "Choose the fitted lifetime shown in the plot legend. "
                "τ1 + τ2 displays both biexponential components; monoexponential fallback datasets show a single τ."
            ),
        )
...
- **TRPL:** skips the first 10 rows, trims to the intensity maximum, normalizes, fits biexponential decay with monoexponential fallback, and reports all fitted lifetimes; the plot legend can display either τ̄ or τ1 + τ2.
