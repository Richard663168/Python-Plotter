...
st.title("PYRL Web")
st.caption("Browser-based PYRL plotting — v0.2")
...
def parse_pl(file_bytes: bytes, filename: str) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    ...
    return {Path(filename).stem: (x_vals, y_vals)}
...
def parse_el_tables(files, time_mode: bool = False) -> Dict[str, pd.DataFrame]:
    ...
    for f in files:
        name = _unique_name(Path(f.name).stem, result)
...
with general_tab:
    if not available_names:
        ...
        if preset == "EL Spectrum" and spectrum_file is not None:
            default_title = Path(spectrum_file.name).stem
        elif file_list:
            default_title = Path(file_list[-1].name).stem
        else:
            default_title = ""
        graph_title = st.text_input("Graph title", value=default_title)

        # Exact V9.2 palette strip.
        st.subheader("Line style & PYRL color palette")
        palette_html = "".join(
            f'<span title="{c}" style="display:inline-block;width:22px;height:22px;background:{c};'
            'border:1px solid #999;margin:2px;border-radius:3px"></span>'
            for c in PRESET_COLORS
        )
        st.markdown(palette_html, unsafe_allow_html=True)
        st.caption("Datasets are automatically assigned sequential colors from your V9.2 swatch palette.")
        ...
        if selected:
            ...
            for i, name in enumerate(selected):
                ...
                with cols[i % 2].expander(name, expanded=False):
                    palette_options = PRESET_COLORS + ["Custom…"]
                    palette_choice = st.selectbox(
                        "Preset color",
                        palette_options,
                        index=i % len(PRESET_COLORS),
                        key=f"palette_{preset}_{name}",
                    )
                    if palette_choice == "Custom…":
                        ...
                        line_color = palette_choice
                        st.markdown(
                            f'<div style="height:18px;width:100%;background:{line_color};border:1px solid #aaa;'
...
- **Colors:** exact 30-swatch V9.2 palette is integrated and automatically assigned to selected datasets.
