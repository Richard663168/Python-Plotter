...
st.title("PYRL Web")
st.caption("Browser-based PYRL plotting — v0.2.1")
...
with general_tab:
    if not available_names:
        ...
        for group_name, group in palette_groups:
            guide_parts.append(
                f'<div style="margin:5px 0 2px 0;font-size:0.85rem;font-weight:600">{group_name}</div>'
            )
            guide_parts.append('<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:6px">')
            for label, hex_value in group:
                short_label = label.replace(f"{group_name} — ", "") if group_name == "Core" else label.split(" — ")[0]
                guide_parts.append(
                    f'<div title="{label}" style="text-align:center;min-width:54px">'
                    f'<div style="height:24px;background:{hex_value};border:1px solid #999;border-radius:3px"></div>'
                    f'<div style="font-size:0.68rem;color:#666;margin-top:1px">{short_label}</div>'
                    '</div>'
                )
