"""Python Plotter v1.0.9

Browser-based Python Plotter, developed by Richard Li.

Streamlit conversion of the original PYRL V9.2.

Supported presets in v1.0:
- UV-VIS
- UV-VIS (eV)
- XRD
- CD
...
st.set_page_config(
    page_title="Python Plotter",
    page_icon="📈",
    layout="wide",
    initial_sidebar_state="expanded",
)

st.title("Python Plotter")
st.caption("Scientific data plotting and processing · Developed by Richard Li · v1.0.7")
...
def sanitize_export_filename(name: str, fallback: str = "exported_plot") -> str:
    """Make a graph title safe to use as a downloaded filename."""
    text = str(name or "").strip()
    if not text:
        text = fallback
    # Replace path separators and characters that commonly cause filesystem issues.
...
def _linear_crossing_x(x0: float, y0: float, x1: float, y1: float, target: float) -> float:
    """Return the x position where the line segment crosses target y."""
    if not all(np.isfinite(v) for v in (x0, y0, x1, y1, target)):
        return np.nan
    if np.isclose(y1, y0):
...
def analyze_pl_spectrum(x: np.ndarray, y: np.ndarray) -> dict:
    """Peak/FWHM metrics using the crossings nearest the global PL maximum."""
    ...
    y = y[mask]
    if x.size < 3:
        raise ValueError("Need at least three valid PL points for peak/FWHM analysis.")
    ...
    if peak_idx > 0:
        ...
        if left_candidates:
            j = left_candidates[-1]  # crossing closest to peak
            left_x = _linear_crossing_x(x[j], y[j], x[j + 1], y[j + 1], half_y)

    right_x = np.nan
    if peak_idx < len(x) - 1:
        for j in range(peak_idx, len(x) - 1):
            a = y[j] - half_y
            b = y[j + 1] - half_y
            if a == 0 or b == 0 or a * b < 0:
                right_x = _linear_crossing_x(x[j], y[j], x[j + 1], y[j + 1], half_y)
                break  # crossing closest to peak

    left_hwhm = peak_x - left_x if np.isfinite(left_x) else np.nan
    right_hwhm = right_x - peak_x if np.isfinite(right_x) else np.nan
    fwhm = right_x - left_x if np.isfinite(left_x) and np.isfinite(right_x) else np.nan

    return {
        "Peak wavelength (nm)": peak_x,
        "Peak intensity": peak_y,
        "Half-max intensity": half_y,
        "Left half-max λ (nm)": left_x,
        "Right half-max λ (nm)": right_x,
        "Left HWHM (nm)": left_hwhm,
        "Right HWHM (nm)": right_hwhm,
        "FWHM (nm)": fwhm,
    }


def analyze_uvvis_peaks(
    x: np.ndarray,
    y: np.ndarray,
    energy: bool = False,
    prominence: float = 0.0,
    min_separation_x: float = 0.0,
    smooth_window: int = 1,
    search_min_x: float | None = None,
    search_max_x: float | None = None,
) -> list[dict]:
    """Detect all local UV-Vis absorbance maxima and return one row per peak.

    Peak finding is performed on an optionally smoothed copy of the spectrum;
    reported absorbance values always come from the original, unsmoothed data.
    """
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    mask = np.isfinite(x) & np.isfinite(y)
    ...
    y = y[mask]
    if x.size < 3:
        raise ValueError("Need at least three valid UV-Vis points for peak analysis.")

    # Sort in increasing x so peak separation has a consistent physical meaning.
    order = np.argsort(x)
    x = x[order]
    y = y[order]

    # Restrict peak detection to the requested search region. The full spectrum
    # is still plotted elsewhere; this mask affects only the peak-summary table.
    if search_min_x is not None or search_max_x is not None:
        lo = -np.inf if search_min_x is None else float(search_min_x)
        hi = np.inf if search_max_x is None else float(search_max_x)
        if hi < lo:
            lo, hi = hi, lo
        region = (x >= lo) & (x <= hi)
        x = x[region]
        y = y[region]
        if x.size < 3:
            raise ValueError("Peak-search range contains fewer than three valid UV-Vis points.")

    y_detect = y.copy()
    w = int(max(1, smooth_window))
    if w > 1 and y_detect.size >= 5:
        if w % 2 == 0:
            w += 1
        w = min(w, y_detect.size if y_detect.size % 2 == 1 else y_detect.size - 1)
        if w >= 3:
            # A low-order Savitzky-Golay filter preserves peak locations better
            # than a moving average. It is used only for detection.
            poly = min(2, w - 1)
            y_detect = savgol_filter(y_detect, window_length=w, polyorder=poly, mode="interp")

    # Convert the requested separation in x-units into a point distance using
    # the median sampling interval. find_peaks requires an integer distance.
    distance_pts = None
    if min_separation_x > 0 and x.size > 1:
        dx = np.diff(x)
        dx = np.abs(dx[np.isfinite(dx) & (dx != 0)])
        if dx.size:
            median_dx = float(np.median(dx))
            distance_pts = max(1, int(np.ceil(float(min_separation_x) / median_dx)))
    ...
    peak_indices, props = find_peaks(y_detect, **kwargs)
    if peak_indices.size == 0:
        return []

    position_label = "Peak energy (eV)" if energy else "Peak wavelength (nm)"
    rows = []
    for n, idx in enumerate(peak_indices, 1):
        row = {
            "Peak #": n,
            position_label: float(x[idx]),
            "Absorbance at peak": float(y[idx]),
        }
        # Prominence is useful for judging whether a detected local maximum is
        # meaningful. If the user did not request a prominence threshold, ask
        # scipy to calculate it after detection for reporting only.
        rows.append(row)

    # Report prominences without changing which peaks were found.
    try:
        from scipy.signal import peak_prominences
        prominences = peak_prominences(y_detect, peak_indices)[0]
        for row, p in zip(rows, prominences):
            row["Prominence"] = float(p)
    except Exception:
        pass
...
def trpl_fit(t: np.ndarray, intensity: np.ndarray) -> dict:
    ...
    # NumPy compatibility: np.trapz was removed in newer NumPy releases.
    # Avoid using it as the default argument to getattr(), because default
    # arguments are evaluated eagerly even when np.trapezoid exists.
    integrate_trapezoid = getattr(np, "trapezoid", None)
    if integrate_trapezoid is None:
        integrate_trapezoid = np.trapz
    area = integrate_trapezoid(np.clip(intensity - C, 0.0, None), t)
...
def plot_cie_dataset(ax, df: pd.DataFrame, name: str, color: str, display_label: Optional[str] = None):
    ...
    ax.scatter(
        ...
        label=(display_label if display_label is not None else name), zorder=10,
    )
    ...
    *,
    ...
    if mode == "2D":
        ...
        for name in selected:
            ...
            display_label = style.get("legend_label", name)
            ax.plot(
                x, y,
                label=f"{display_label}   λ≈{used_wl:.1f} nm",
...
def build_figure(
    *,
    ...
    for ax, name in targets:
        ...
        color = style["line_color"]
        marker = None if style["marker"] == "None" else style["marker"]
        custom_legend_label = style.get("legend_label", name)
        ...
        if preset == "TRPL":
            ...
            ax.plot(
                ...
                color=color, label=custom_legend_label,
            )
            ax.plot(
                d["fit_t"], d["fit_I"],
            ...
            plot_cie_dataset(ax, cie_frames[name], name, color, custom_legend_label)
        else:
            ...
            display_label = custom_legend_label
            if preset == "EL Spectrum" and custom_legend_label == name:
                # Preserve the existing automatic Voltage/Current/J/L legend conversion
                # until the user explicitly edits this dataset's legend name.
            ...
            if secondary_datasets and name in secondary_datasets:
                ...
                ax2.plot(
                    ...
                    markerfacecolor=style["marker_color"],
                    markeredgecolor=style["marker_color"],
                    label=f"{display_label} ({y2_label_override or 'Y2'})",
                )
    ...
    if title:
        axes[0].set_title(title, fontsize=title_fontsize, fontweight="bold", pad=10)
    ...
    fig.tight_layout(rect=[0.03 if plot_mode == "Stacked" else 0, 0, 1, 1])
    return fig
...
general_tab, text_tab, limit_tab, misc_tab, background_tab = st.tabs(
    ["General", "Text", "Limit", "Misc", "Background"]
)
...
trpl_lifetime_display = "τavg"

with general_tab:
    # Preset-specific processing controls that determine which datasets exist.
    if preset in ("UV-VIS", "UV-VIS (eV)") and simple_datasets:
        st.subheader("UV–Vis Peak Results")
        with st.expander("UV–Vis peak summary", expanded=True):
            st.caption(
                "Detects local absorbance maxima (rising → peak → falling). "
                "Smoothing is used only for peak detection; reported absorbance values come from the raw spectrum."
            )
            # Peak-search window is always entered in wavelength (nm), even for
            # the eV display preset. This keeps the control intuitive and makes
            # it easy to isolate the excitonic/visible region.
            wl_values = []
            for _x, _y in simple_datasets.values():
                _x = np.asarray(_x, dtype=float)
                _x = _x[np.isfinite(_x) & (_x > 0)]
                if _x.size:
                    if preset == "UV-VIS (eV)":
                        wl_values.extend((1240.0 / _x).tolist())
                    else:
                        wl_values.extend(_x.tolist())
            if wl_values:
                wl_data_min = float(np.nanmin(wl_values))
                wl_data_max = float(np.nanmax(wl_values))
            else:
                wl_data_min, wl_data_max = 200.0, 1000.0

            rc1, rc2 = st.columns(2)
            with rc1:
                uv_peak_wl_min = st.number_input(
                    "Peak-search wavelength min (nm)",
                    value=wl_data_min,
                    step=10.0,
                    format="%.2f",
                    key=f"uv_peak_wl_min_{preset}",
                    help="Only peaks at or above this wavelength are included in the table. The plotted spectrum is unchanged.",
                )
            with rc2:
                uv_peak_wl_max = st.number_input(
                    "Peak-search wavelength max (nm)",
                    value=wl_data_max,
                    step=10.0,
                    format="%.2f",
                    key=f"uv_peak_wl_max_{preset}",
                    help="Only peaks at or below this wavelength are included in the table. The plotted spectrum is unchanged.",
                )

            pc1, pc2, pc3 = st.columns(3)
            with pc1:
                uv_peak_prominence = st.number_input(
                    "Minimum prominence",
                    min_value=0.0,
                    value=0.0,
                    step=0.0001,
                    format="%.6f",
                    key=f"uv_peak_prominence_{preset}",
                    help="0 detects every local maximum. Increase this to reject small noise peaks.",
                )
            with pc2:
                uv_peak_separation = st.number_input(
                    "Minimum peak separation",
                    min_value=0.0,
                    value=3.0,
                    step=1.0,
                    format="%.3f",
                    key=f"uv_peak_separation_{preset}",
                    help="Minimum separation between reported peaks, in nm for UV-VIS or eV for UV-VIS (eV).",
                )
            with pc3:
                uv_peak_smooth = int(st.number_input(
                    "Detection smoothing window (points)",
                    min_value=1,
                    value=5,
                    step=2,
                    key=f"uv_peak_smooth_{preset}",
                    help="Odd Savitzky-Golay window used only to locate peaks. Set to 1 for no smoothing.",
                ))

            rows = []
            for name, (x, y) in simple_datasets.items():
                try:
                    if float(uv_peak_wl_max) <= float(uv_peak_wl_min):
                        raise ValueError("Peak-search wavelength max must be larger than min.")

                    if preset == "UV-VIS (eV)":
                        # Energy decreases with wavelength, so wavelength limits
                        # reverse when converted to the eV x-axis.
                        search_min_x = 1240.0 / float(uv_peak_wl_max)
                        search_max_x = 1240.0 / float(uv_peak_wl_min)
                    else:
                        search_min_x = float(uv_peak_wl_min)
                        search_max_x = float(uv_peak_wl_max)

                    peak_rows = analyze_uvvis_peaks(
                        x, y,
                        energy=(preset == "UV-VIS (eV)"),
                        prominence=float(uv_peak_prominence),
                        min_separation_x=float(uv_peak_separation),
                        smooth_window=uv_peak_smooth,
                        search_min_x=search_min_x,
                        search_max_x=search_max_x,
                    )
                    if not peak_rows:
                        st.warning(f"{name}: no peaks detected with the current settings.")
                    for peak_row in peak_rows:
                        rows.append({"Dataset": name, **peak_row})
                except Exception as exc:
                    st.warning(f"{name}: {exc}")
            if rows:
                uv_metrics_df = pd.DataFrame(rows)
                numeric_cols = [c for c in uv_metrics_df.columns if c != "Dataset"]
                uv_metrics_df[numeric_cols] = uv_metrics_df[numeric_cols].round(4)
                st.dataframe(uv_metrics_df, hide_index=True, use_container_width=True)
            st.caption(
                "A peak is a local maximum. For a smooth spectrum this corresponds to the first derivative changing "
                "from positive to negative. The wavelength window limits where peaks are searched; "
                "prominence/separation suppress small noise maxima."
            )
        available_names = list(simple_datasets.keys())

    elif preset == "PL" and simple_datasets:
        st.subheader("PL Spectrum Results")
        with st.expander("PL peak & FWHM summary", expanded=True):
            rows = []
            for name, (x, y) in simple_datasets.items():
                try:
                    row = {"Dataset": name}
                    row.update(analyze_pl_spectrum(x, y))
                    rows.append(row)
                except Exception as exc:
                    st.warning(f"{name}: {exc}")
            if rows:
                pl_metrics_df = pd.DataFrame(rows)
                numeric_cols = [c for c in pl_metrics_df.columns if c != "Dataset"]
                pl_metrics_df[numeric_cols] = pl_metrics_df[numeric_cols].round(3)
                st.dataframe(pl_metrics_df, hide_index=True, use_container_width=True)
            st.caption(
                "Half-max crossings are the crossings nearest the PL peak and are linearly interpolated "
                "between adjacent wavelength points. FWHM = right crossing − left crossing."
            )
        available_names = list(simple_datasets.keys())
        ...
        st.subheader("TRPL Fit Results")
        with st.expander("TRPL fit summary", expanded=True):
            rows = []
            for name, d in trpl_processed.items():
                ...
                if params.get("model") == "bi":
                    row.update({
                        "A1": params.get("A1"),
                        "τ1 (ns)": params.get("tau1"),
                        "A2": params.get("A2"),
                        "τ2 (ns)": params.get("tau2"),
                    })
                else:
                    row.update({
                        "A1": params.get("A"),
                        "τ1 (ns)": params.get("tau"),
                    })
                rows.append(row)
            st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
            st.caption("A1 and A2 are fitted amplitudes for the normalized TRPL decay. Monoexponential fallback fits use A1 and τ1 only.")
        available_names = list(simple_datasets.keys())
    ...
    if not available_names:
        ...
        dataset_signature = hashlib.md5("|".join(map(str, available_names)).encode("utf-8")).hexdigest()[:10]
        selected = st.multiselect(
            "Datasets", available_names, default=available_names,
            key=f"datasets_{preset}_{dataset_key_suffix}_{dataset_signature}",
        )
        ...
        # Title default mirrors V9.2's last selected filename behavior, but only
        # auto-updates on a new upload if the user has not manually customized it.
        ...
        title_key = f"graph_title_{preset}"
        source_key = f"graph_title_auto_source_{preset}"
        previous_source = st.session_state.get(source_key, None)
        current_title = st.session_state.get(title_key, None)
        if current_title is None:
            st.session_state[title_key] = default_title
        elif previous_source != default_title and (not current_title or current_title == previous_source):
            st.session_state[title_key] = default_title
        st.session_state[source_key] = default_title

        title_col, reset_col = st.columns([5, 1])
        with reset_col:
            if st.button("Use filename", key=f"title_filename_{preset}", use_container_width=True):
                st.session_state[title_key] = default_title
        with title_col:
            graph_title = st.text_input("Graph title", key=title_key)

        # Exact V9.2 palette, labeled so no hex memorization is needed.
        st.subheader("Line style & color")
        with st.expander("PYRL color palette reference", expanded=False):
            palette_groups = [
        ...
        st.caption("Datasets receive stable sequential PYRL colors. Open the palette reference only when you need it.")
        ...
        if selected:
            ...
            for i, name in enumerate(selected):
                try:
                    palette_index = available_names.index(name)
                except ValueError:
                    palette_index = i
                default_color = PRESET_COLORS[palette_index % len(PRESET_COLORS)]
                ...
                with cols[i % 2].expander(name, expanded=False):
                    legend_label = st.text_input(
                        "Legend name",
                        value=name,
                        key=f"legend_label_{preset}_{name}",
                        help="Changes only the displayed legend text; the underlying dataset/file name is unchanged.",
                    )
                    ...
                    styles[name] = {
                        "legend_label": legend_label.strip() if legend_label.strip() else name,
    ...
    try:
        ...
        if preset == "TR CP-EL":
            ...
            fig = build_figure(
                ...
                trpl_lifetime_display=trpl_lifetime_display,
                styles=styles,
                plot_mode=plot_mode,
                title=graph_title,
                fig_width=float(fig_width),
        ...
        download_name = f"{sanitize_export_filename(graph_title)}.png"
        st.download_button(
...
with st.expander("v1.0 compatibility notes"):
    st.markdown(
        """
- **UV-VIS / UV-VIS (eV):** paired-column V9.2 layout; filters wavelength ≤ 200 nm; peak summary reports the maximum absorbance position and absorbance.
...
- **TRPL:** skips the first 10 rows, trims to the intensity maximum, normalizes, fits biexponential decay with monoexponential fallback, and reports all fitted lifetimes; the fit table reports τ1, τ2, fitted amplitudes A1/A2, τ̄ fit, and integral lifetime; plot legends remain dataset-only.
