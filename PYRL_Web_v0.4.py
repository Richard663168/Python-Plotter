...
with giwaxs_tab:
    st.info(
        ...
        "v0.4 adds CP–EL, gₑₗ, TR CP–EL, and TRPL while preserving the v0.3 spectroscopy tools."
    )
...
- **gₑₗ:** pairs RCP/LCP CP–EL files from the first filename block and calculates `2(IL−IR)/(IL+IR)` after wavelength-grid alignment.
...
- **Preset typography:** the native dropdown uses `gₑₗ` with true Unicode subscript glyphs. `g₍CD₎` and `g₍CPPL₎` use the closest Unicode subscript-style notation because Unicode lacks subscript C/D/P letters; plotted axis labels use proper math subscripts.
